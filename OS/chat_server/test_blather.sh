...
ticktime_valg=0.10              # ticks during valgrind - longer due to debug delays
ticktime=$ticktime_valg         # global tick time, set during normal/valgrind
startticks=5

echo "ticktime_norm: $ticktime_norm"
echo "ticktime_valg: $ticktime_valg"
...
unset BL_ADVANCED                                                  # disable advanced features

printf "RUNNING TESTS without Valgrind\n"
mkdir -p $TESTDIR
cd $TESTDIR
...
if [ "$run_norm" = "1" ]; then
    ...
    for i in $all_tests; do
        ...
        outfile=$server_out                                        # check the server output file
        printf "%s\n" "${expect_server[i]}" > ${outfile}.expect
        if ! $DIFF ${outfile}.expect $outfile > ${outfile}.diff
        then
            printf "FAIL\n"
            minor_sep 
            printf "Server output does not match expecte\n"
        ...
        for ((c=0; c<${#outfiles[*]}; c++)); do                    # check each client output file
            outfile=${outfiles[c]}
            printf "%s\n" "${expect_client_outs[i]}" | awk -F '\t' "{print \$$((c+1))}" > ${outfile}.expect
